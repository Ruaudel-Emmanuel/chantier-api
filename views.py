...
from .filters import ChantiersFilter, TacheFilter
from .models import (
    Anomalie,
    Chantier,
    Equipe,
...
from .permissions_filters import IsChefOrReadOnly
from .serializers import (
    AnomalieSerializer,
    ChantiersDetailSerializer,
    ChantiersSerializer,
    EquipeSerializer,
    HeuresTravailSerializer,
    LotSerializer,
    MembreSerializer,
...
class ChantiersViewSet(viewsets.ModelViewSet):
    ...
    queryset = Chantier.objects.filter(
        actif=True
    ).prefetch_related('lots__taches')
    ...
    def perform_create(self, serializer):
        ...
        logger.info(
            f"Chantier créé : {serializer.instance.numero} "
    ...
    def rapport(self, request, pk=None):
        ...
        taches_terminees = sum(
            lot.taches.filter(status='TERMINEE').count()
            for lot in lots
        )

        heures_estimees = sum(
            lot.taches.aggregate(
                Sum('heures_estimees')
            )['heures_estimees__sum'] or 0
            for lot in lots
        )
        ...
        heures_reelles = sum(
            lot.taches.aggregate(
                Sum('heures_reelles')
            )['heures_reelles__sum'] or 0
            for lot in lots
        )

        progression = (
            (taches_terminees / taches_totales * 100)
            if taches_totales else 0
        )
...
class TachesViewSet(viewsets.ModelViewSet):
    ...
    - PATCH /api/v1/taches/{id}/ → Mettre à jour status, heures
    - POST /api/v1/taches/{id}/photo/ → Ajouter une photo
    - GET /api/v1/taches/{id}/heures/ → Historique des heures
    - POST /api/v1/taches/{id}/heures/ → Enregistrer heures (MOBILE)
    ...
    def photo(self, request, pk=None):
        ...
        if serializer.is_valid():
            ...
            logger.info(
                f"Photo uploadée : {photo.id} "
                f"pour tâche {tache.numero}"
            )
            return Response(
    ...
    def heures(self, request, pk=None):
        ...
        if request.method == 'GET':
            ...
            if serializer.is_valid():
                ...
                logger.info(
                    f"Heures enregistrées : {heures.heures}h "
                    f"pour {tache.numero}"
                )
                return Response(
    ...
    def signaler_anomalie(self, request, pk=None):
        ...
        if serializer.is_valid():
            ...
            logger.warning(
                f"Anomalie signalée [{anomalie.severite}] : "
                f"{anomalie.titre}"
            )
            return Response(
...
class HeuresTravailViewSet(viewsets.ModelViewSet):
    ...
    def mes_heures(self, request):
        ...
        fin_mois = (
            debut_mois + relativedelta(months=1)
        ...
        serializer = HeuresTravailSerializer(heures, many=True)
        total = sum(
            h['heures'] for h in serializer.data
            if 'heures' in h
        )
...
class EquipesViewSet(viewsets.ModelViewSet):
    ...
    queryset = Equipe.objects.filter(
        actif=True
...
class AnomaliesViewSet(viewsets.ModelViewSet):
    ...
    def assigner(self, request, pk=None):
        ...
        try:
            ...
            logger.info(
                f"Anomalie assignée : {anomalie.id} à {responsable}"
            )
            return Response(
                AnomalieSerializer(anomalie).data,
                status=status.HTTP_200_OK
