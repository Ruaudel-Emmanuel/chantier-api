...
class StatusTache(models.TextChoices):
    ...
    EN_COURS = 'EN_COURS', 'En cours'
    EN_ATTENTE = 'EN_ATTENTE', 'En attente (bloqué)'
...
class Chantier(models.Model):
    ...
    nom = models.CharField(
        max_length=200,
        help_text="Nom/description du chantier"
    )

    # Localisation
    adresse = models.CharField(
        max_length=255,
        help_text="Adresse complète du chantier"
    )
    ...
    notes_internes = models.TextField(
        blank=True,
        help_text="Notes non visibles au client"
    )
...
class Lot(models.Model):
    ...
    numero = models.PositiveIntegerField(
        help_text="Numéro d'ordre (1, 2, 3...)"
    )
...
class Tache(models.Model):
    ...
    numero = models.CharField(
        max_length=50,
        help_text="Numéro de tâche (T-001, T-002...)"
    )
    ...
    bloquee_par = models.ManyToManyField(
        ...
        blank=True,
        related_name='bloque',
        help_text="Tâches qui bloquent celle-ci"
    )
    ...
    def calculer_heures_reelles(self):
        """Met à jour heures_reelles en sommant les entrées."""
        total = self.heures_travail.aggregate(
            Sum('heures')
        )['heures__sum'] or 0
    ...
    def est_en_retard(self):
        ...
        if self.status == StatusTache.TERMINEE:
            if (self.date_fin_reelle and
                    self.date_fin_reelle > self.date_fin_prevue):
                return True
        else:
...
class HeureTravail(models.Model):
    ...
    def __str__(self):
        return (
            f"{self.tache.numero} - {self.membre} - "
            f"{self.heures}h le {self.date}"
        )
...
class PhotoRapport(models.Model):
    ...
    def __str__(self):
        date_str = self.date_photo.strftime('%d/%m/%Y')
        return f"Photo - {self.tache.numero} - {date_str}"
...
# ⚠️ IMPORTANT : Toujours à la FIN après toutes les classes
# =================================================================


@receiver(post_save, sender=HeureTravail)
def maj_heures_tache(sender, instance, **kwargs):
    """Signal : Mettre à jour heures_reelles quand on ajoute."""
...
def log_changement_chantier(sender, instance, created, **kwargs):
    ...
    if created:
        logger.info(
            f"Chantier créé : {instance.numero} - {instance.nom}"
        )
    else:
